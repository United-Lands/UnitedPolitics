...
public class AdminSetReputationCommand extends BaseCommandHandler<UnitedPolitics> {

    public AdminSetReputationCommand(UnitedPolitics plugin, IMessageProvider messageProvider) {
    ...
    public void handleCommand(CommandSender sender, String[] args) {
        if (args.length < 4) {
            Messenger.sendMessage(sender, messageProvider.getList("messages.usages.reputation.set"), null,
        ...
        if (plugin.getReputationManager().addOrUpdateReputationScoreEntry(entry)) {
            Messenger.sendMessage(sender, messageProvider.get("messages.success.reputation.set"),
