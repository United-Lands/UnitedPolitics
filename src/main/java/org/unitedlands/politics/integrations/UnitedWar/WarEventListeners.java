...
public class WarEventListeners implements Listener {
    ...
    public void onWarDeclared(WarDeclaredEvent event) {
        ...
        if (event.isNationWar()) {
            if (attacker.getNation() != null) {
                attackerNationTowns.addAll(attacker.getNation().getTowns());
            }
        }

        var defender = GeopolUtils.findGeopolObject(event.getTargetTownId());
        ...
        for (var town : towns) {
            ...
            if (score >= friendsThreshold) {
                if (attackerNationTowns.isEmpty()) {
                    plugin.getReputationManager().handleReputationChange(town, attacker, penalty, "uw-warred-friend",
                            null, false);
                } else {
        ...
        for (var nation : nations) {
            ...
            if (score >= friendsThreshold) {
                if (attackerNationTowns.isEmpty()) {
                    plugin.getReputationManager().handleReputationChange(nation, attacker, penalty, "uw-warred-friend",
                            null, false);
                } else {
                    for (var attackerTown : attackerNationTowns) {
